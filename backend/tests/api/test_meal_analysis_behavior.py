...
@pytest.mark.usefixtures("auth_overrides")
def test_meal_analysis_openai_unavailable_does_not_break_endpoint(client, monkeypatch):
    async def _generate_json(self, *, prompt, schema):
        _ = (self, prompt, schema)
        return {"text": "", "items": [], "warnings": ["openai_unavailable"]}

    monkeypatch.setattr(OpenAIClient, "generate_json", _generate_json)
    ...
    assert response.status_code == 200
    assert response.json()["data"]["warnings"] == ["openai_unavailable"]
    assert "Estimated from visible portions" in response.json()["data"]["explanation"]
