...
async def me(current_user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)) -> dict:
    ...
    dto = MeDTO(
        ...
        profile=UserProfileDTO.model_validate(profile, from_attributes=True) if profile else None,
    )
    return success_response(data=serialize_api_data(dto.model_dump()))
...
async def update_profile(payload: UserProfileUpdateRequest, current_user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)) -> dict:
    ...
    return success_response(data=serialize_api_data(UserProfileDTO.model_validate(profile, from_attributes=True).model_dump()))
...
async def update_locale(payload: LocaleUpdateRequest, current_user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)) -> dict:
    ...
    return success_response(data=serialize_api_data({"locale": user.locale if user else current_user.locale, "timezone": user.timezone if user else current_user.timezone}))
