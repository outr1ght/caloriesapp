...
class UploadService:
    ...
    async def init_upload(self, user_id: str, payload: UploadInitRequest) -> dict:
        validate_upload(filename=payload.filename, mime_type=payload.mime_type, file_size=payload.file_size, sha256=payload.sha256)
        upload_id = str(uuid4())
        ext = payload.filename.split(".")[-1].lower()
        storage_key = f"users/{user_id}/uploads/{upload_id}.{ext}"
        image = UploadedImage(id=upload_id, user_id=user_id, meal_id=payload.meal_id, storage_key=storage_key, mime_type=payload.mime_type.lower(), file_size=payload.file_size, sha256=payload.sha256.lower())
        self.session.add(image)
        await self.session.commit()
        presigned = self.storage.create_presigned_upload(key=storage_key, mime_type=payload.mime_type.lower())
        return {"upload_id": upload_id, "storage_key": storage_key, "upload_url": presigned["upload_url"], "upload_headers": presigned["upload_headers"], "expires_at": presigned["expires_at"]}
